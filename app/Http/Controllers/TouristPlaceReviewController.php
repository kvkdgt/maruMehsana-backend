...
use App\Models\TouristPlaceReview;
...
class TouristPlaceReviewController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $existing = TouristPlaceReview::where('tourist_place_id', $request->tourist_place_id)
            ->where('app_user_id', $request->user_id)
            ->first();

        if ($existing) {
            return response()->json(['status' => 'error', 'message' => 'You have already reviewed this place.'], 400);
        }

        $review = TouristPlaceReview::create([
    ...
    {
        $review = TouristPlaceReview::findOrFail($id);
    ...
    /**
     * Get reviews for a tourist place.
     */
    public function getPlaceReviews($placeId)
    {
        $reviews = TouristPlaceReview::where('tourist_place_id', $placeId)
